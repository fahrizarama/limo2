...
class Mdl_produk extends CI_Model {
	...
	var $column_order = array('id_produk','produk_utama',null); //set column field database for datatable orderable
	var $column_search = array('id_produk','produk_utama'); //set column field database for datatable searchable just title , author , category are searchable
	...
	function update_data_2($where,$data,$table){
	    	$this->db->where($where);
		    $this->db->update($table,$data);
	}
